...
// Build app using the mock orchestrator (simple unit tests)
...
// Build app wired to mocked GeminiService (integration-style tests)
...
describe("POST /agent/run — unit (mock orchestrator)", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("returns 200 with answer, confidence, and nextAction for a valid request", async () => {
    const res = await request(buildMockApp())
      .post("/agent/run")
  ...
  it("returns the exact values from the orchestrator result", async () => {
    const res = await request(buildMockApp())
      .post("/agent/run")
      .send({ prompt: "Explain the builder readiness model", appId: "school-of-base" });
  ...
  it("calls orchestrator.run with prompt, appId, mode, and empty context by default", async () => {
    await request(buildMockApp())
      .post("/agent/run")
  ...
  it("passes context and mode through to orchestrator when provided", async () => {
    const ctx = { role: "admin" };
    await request(buildMockApp())
      .post("/agent/run")
      .send({ prompt: "Hello", appId: "my-app", mode: "teacher", context: ctx });
...
describe("POST /agent/run — integration (mocked GeminiService)", () => {
  beforeEach(() => {
    MockedGeminiService.mockClear();
  });

  it("returns 200 with full result when GeminiService responds", async () => {
    ...
    const res = await request(buildIntegrationApp())
      .post("/agent/run")
  ...
  it("returns 500 when GeminiService throws (no API key)", async () => {
    MockedGeminiService.prototype.generateContent = jest
      .fn()
